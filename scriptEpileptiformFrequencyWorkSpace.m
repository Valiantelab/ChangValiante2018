...
eventTimes = int64(events(indexEvent,1:2));
event = cell(numel(events(:,1)),1);   %preallocate cell array

%Make vectors of SLEs
for i = 1:numel(eventTimes(:,1))
    epileptiformEvent{i,1} = LFP_filtered(eventTimes(i,1)*frequency:eventTimes(i,2)*frequency);         
end
