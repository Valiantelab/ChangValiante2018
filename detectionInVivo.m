function [spikes, events, SLE, details, artifactSpikes] = detectionInVivo(FileName, userInput, x, samplingInterval, metadata)
% inVivoDetection is a function designed to search for epileptiform
% events from the in vivo mouse model (Kindled seizures)
...
if indexSLE
    ...
    if mean(intensity) >= 0.49  %If below, likely contaminated by noise, issue a warning to End User to review raw data.
        fprintf(1,'\nSuccessfully completed file: %s using The In Vivo Mouse Model Epileptiform Detector %s.\n', FileName, finalTitle)
    else
