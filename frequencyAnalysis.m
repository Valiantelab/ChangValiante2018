...
if troubleshooting 
    for i = 1:numel(eventTimes(:,1))

    %Set variables    
    startLocation = int64(eventTimes(i,1)*frequency);
    endLocation = int64(eventTimes(i,2)*frequency);
    ...
    [s,f,t] = spectrogram (eventVector, 10*frequency, 9.5*frequency, [], frequency, 'yaxis');
    ...
    contour(t,f,abs(s).^2)
    c = colorbar;
    c.Label.String = 'Power (Watt)';    %what is the unit really called? 
    ylim([0 20])
