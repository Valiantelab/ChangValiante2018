...
diffLFP_normalized = diff(LFP_normalizedFiltered);

%%

figure;
subplot (2,1,1)
plot(LFP_normalizedLowPassFiltered(time1:time2))
subplot (2,1,2)
plot(LFP_normalized(time1:time2))


%test analysis - power analysis on filtered LFP
...
onsetBaseline = single(time1-50000);
offsetBaseline = single(time2+50000);

%
onsetSLE = (SLE(i,1)*10000);
offsetSLE = (SLE(i,2)*10000);

onsetBaseline = (time1-50000);
offsetBaseline = (time2+50000);
%

onsetContext = onsetBaseline:onsetSLE;
offsetContext = offsetBaseline:offsetSLE;

%variables for plotting
data1=DiffLFP_normalizedFiltered;

%calculating the power of the data
powerAnalysis = (data1(onsetContext)).^2;
